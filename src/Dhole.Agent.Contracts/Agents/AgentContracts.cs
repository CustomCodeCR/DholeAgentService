...
public sealed record AgentScheduleDto(Guid Id,string Name,Guid AgentDefinitionId,Guid ProviderId,Guid? CredentialId,string ScheduleType,string? CronExpression,int? IntervalMinutes,DateTime? ExecuteAt,string Timezone,string InputJson,bool IsActive,DateTime? LastExecutionAt,DateTime? NextExecutionAt,int MaxRetries,int TimeoutSeconds);
public sealed record CreateAgentScheduleRequest(string Name,Guid AgentDefinitionId,Guid ProviderId,Guid? CredentialId,string ScheduleType,string? CronExpression,int? IntervalMinutes,DateTime? ExecuteAt,string Timezone,string InputJson,int MaxRetries,int TimeoutSeconds);
public sealed record UpdateAgentScheduleRequest(string Name,Guid? CredentialId,string ScheduleType,string? CronExpression,int? IntervalMinutes,DateTime? ExecuteAt,string Timezone,string InputJson,int MaxRetries,int TimeoutSeconds,DateTime? NextExecutionAt);

public sealed record AgentExecutionDto(Guid Id,Guid AgentDefinitionId,Guid ProviderId,Guid? ScheduleId,Guid? CredentialId,string ExecutionType,string Status,int Priority,string InputJson,string? OutputJson,DateTime? StartedAt,DateTime? CompletedAt,long? DurationMs,int Attempt,int MaxAttempts,string? ErrorCode,string? ErrorMessage,string CorrelationId,string? TraceId,DateTime CreatedAtUtc);
